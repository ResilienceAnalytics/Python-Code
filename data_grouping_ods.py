...
def transform_data(data):
    ...
    return data.groupby('DATE').mean(numeric_only=True).reset_index()

def main(input_file, output_file):
    data = load_data(input_file)
    transformed_data = transform_data(data)
    transformed_data.to_excel(output_file, engine='odf', index=False)
    print(f"Transformed data has been saved to {output_file}")

if __name__ == "__main__":
    if len(sys.argv) < 3:
        print("Usage: python script.py <input_file> <output_file.ods>")
        sys.exit(1)
